/*                                                                         */
/*               InstPubs.SQL - Crea la base de datos Pubs                   */
/*                                                                         */
/*
...
establecer  sin contar     en
establecer  formato de fecha mdy

USO  maestro

declarar @dttm varchar ( 55 )
seleccione   @dttm = convertir ( varchar , getdate (), 113 )
raiserror ( ' Comenzando InstPubs.SQL en %s ....' , 1 , 1 ,@dttm) sin  esperar

IR

si  existe ( seleccione  *  de sysdatabases donde  nombre = ' pubs' )
comenzar
  raiserror ( ' Eliminando la base de datos de pubs existente ....' , 0 , 1 )
   Publicaciones de base de datos DROP
fin
IR
...
raiserror ( ' Creando base de datos de pubs....' , 0 , 1 )
ir
/*
   Usar tamaño predeterminado con crecimiento automático
*/

CREAR  BASE DE DATOS pubs
IR
...
if  db_name () <>  ' publicaciones'
   raiserror ( ' Error en InstPubs.SQL, ' ' USE pubs' ' falló! Eliminando el SPID ahora.'
            , 22 , 127 ) con  registro

IR

si  CAST ( SERVERPROPERTY ( ' ProductMajorVersion' ) AS  INT ) < 12
COMENZAR
  exec sp_dboption ' pubs' , ' trunc. iniciar sesión en chkpt.' , ' verdadero'
  exec sp_dboption ' pubs' , ' select into/bulkcopy' , ' true'
FIN
ELSE  ALTER  DATABASE [pubs] ESTABLECER  RECUPERACIÓN  SIMPLE  CON NO_WAIT
IR

ejecutar sp_addtype id, ' varchar(11)' , ' NO NULO'
ejecutar sp_addtype tid, ' varchar(6)'   , ' NO NULO'
ejecutar sp_addtype empid , ' char(9)'      , ' NOT NULL'

raiserror ( ' Ahora en la sección de creación de tabla ....' , 0 , 1 )

IR

CREAR  TABLA autores
(
   identificación au_id

         COMPROBAR (au_id como  ' [0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9][0-9][0- 9]' )

         CONSTRAINT UPKCL_auidind PRIMARY  KEY  CLUSTERED ,

   au_lname        varchar ( 40 )        NO  NULO ,
   au_fname        varchar ( 20 )        NO  NULO ,

carácter    del teléfono           ( 12 )           NO  NULO

         PREDETERMINADO ( ' DESCONOCIDO' ),

   dirección         varchar ( 40 )            NULL ,
   ciudad            varchar ( 20 )            NULL ,
             carácter de estado ( 2 )                NULL ,

carácter    zip             ( 5 )                NULL

         COMPROBAR (comprimir como  ' [0-9][0-9][0-9][0-9][0-9]' ),

          bit                de contrato NO  NULO
)

IR

 Editores CREAR TABLA
(
   pub_id          char ( 4 )            NO  NULO

         RESTRICCIÓN UPKCL_pubind CLAVE PRIMARIA  AGRUPADA 

         COMPROBAR (pub_id en ( ' 1389' , ' 0736' , ' 0877' , ' 1622' , ' 1756' )
            O pub_id como  ' 99[0-9][0-9]' ),
...
CREAR  títulos de TABLA
(
   título_id tid

         CONSTRAINT UPKCL_titleidind PRIMARY  KEY  CLUSTERED ,

   título           varchar ( 80 )        NO  NULO ,

   tipo            char ( 12 )           NO  NULO

         PREDETERMINADO ( ' INDECIDIDO' ),

   pub_id          char ( 4 )                NULO

         REFERENCIAS editores(pub_id),

   precio           dinero                  NULL ,
   anticipo         de dinero                  NULL ,
   realeza         int                    NULL ,
   ytd_sales       int                    NULL ,
...
CREAR  TABLA títuloautor
(
   identificación au_id

         REFERENCIAS autores(au_id),

   título_id tid

         REFERENCIAS títulos(title_id),

   au_ord          tinyint                NULL ,
   realezaper      int                    NULL ,


   RESTRICCIÓN UPKCL_taind CLAVE PRINCIPAL  EN CLÚSTER (au_id, title_id) 
)
...
IR
 ...
 Tiendas CREAR TABLA
(
   stor_id         char ( 4 )            NO  NULO

         CONSTRAINT UPK_storeid PRIMARY  KEY  CLUSTERED ,

   nombre_almacenamiento       varchar ( 40 )            NULL ,
...
(
   stor_id         char ( 4 )            NO  NULO
         ...
         REFERENCIAS tiendas(stor_id),

   ord_num         varchar ( 20 )        NO  NULO ,
   ord_date        datetime           NOT  NULL ,
   qty             smallint           NOT  NULL ,
   términos de pago        varchar ( 12 )        NO  NULO ,
   ...
   RESTRICCIÓN UPKCL_sales PRIMARY  KEY  CLUSTERED (stor_id, ord_num, title_id)
)

IR

CREAR  TABLA roysched
(
   título_id tid
...
(
   tipo de descuento    varchar ( 40 )        NO  NULO ,
   ...
   alto         número pequeño               NULL ,
   descuento        diciembre ( 4 , 2 )           NO  NULO
)
...
(
   job_id          smallint           IDENTIDAD ( 1 , 1 )

          CLAVE  PRINCIPAL EN CLÚSTER ,

   job_desc        varchar ( 50 )        NO  NULO

         DEFAULT  ' Nueva posición - título aún no formalizado' ,

   min_lvl         tinyint            NO  NULO

         COMPROBAR (min_lvl >=  10 ),

   max_lvl         tinyint            NO  NULO

         COMPROBAR (max_lvl <=  250 )
)

IR

CREAR  TABLA pub_info
(
   pub_id          char ( 4 )            NO  NULO

         REFERENCIAS editores(pub_id)

         RESTRICCIÓN UPKCL_pubinfo CLAVE PRIMARIA  AGRUPADA , 

imagen                     del logotipo            NULL ,
   pr_info         texto                   NULO
)
...
(
   emp_id empid
         ...
         RESTRICCIÓN PK_emp_id CLAVE PRINCIPAL  NO CLÚSTER 

         RESTRICCIÓN CK_emp_id COMPROBAR (emp_id LIKE
            ' [AZ][AZ][AZ][1-9][0-9][0-9][0-9][0-9][FM]'  o
            emp_id LIKE  ' [AZ]-[AZ][1-9][0-9][0-9][0-9][0-9][FM]' ) ,

   fname           varchar ( 20 )        NO  NULO ,
carácter    minit           ( 1 )                NULO ,
   lname           varchar ( 30 )        NO  NULO ,

   job_id          smallint           NO  NULO

         PREDETERMINADO  1

         REFERENCIAS puestos de trabajo (job_id),
   ...
   contratacion_fecha       fechahora           NO  NULO
...
raiserror ( ' Ahora en la sección de creación de disparador ...' , 0 , 1 )
...
raiserror ( ' Ahora en las inserciones a los editores ....' , 0 , 1 )

IR

inserte los valores de los editores ( ' 0736' , ' New Moon Books' , ' Boston' , ' MA' , ' USA' )
inserte los valores de los editores ( ' 0877' , ' Binnet & Hardley' , ' Washington' , ' DC' , ' USA' )
inserte los valores de los editores ( ' 1389' , ' Algodata Infosystems' , ' Berkeley' , ' CA' , ' USA' )
inserte los valores de los editores ( ' 9952' , ' Scootney Books' , ' Nueva York' , ' NY' , ' USA' )
inserte los valores de los editores ( ' 1622' , ' Five Lakes Publishing' , ' Chicago' , ' IL' , ' USA' )
inserte los valores de los editores ( ' 1756' , ' Ramona Publishers' , ' Dallas' , ' TX' , ' USA' )
insertar valores de editores ( ' 9901' , ' GGG&G' , ' M�nchen' , NULL , ' Alemania' )
inserte los valores de los editores ( ' 9999' , ' Lucerne Publishing' , ' Paris' , NULL , ' Francia' )

IR

raiserror ( ' Ahora en las inserciones de pub_info ....' , 0 , 1 )

IR

insertar valores pub_info('0736',,
Estos son datos de texto de muestra para New Moon Books, editorial 0736 en la base de datos de pubs. New Moon Books se encuentra en Boston, Massachusetts.
Estos son datos de texto de muestra para New Moon Books, editorial 0736 en la base de datos de pubs. New Moon Books se encuentra en Boston, Massachusetts.
Estos son datos de texto de muestra para New Moon Books, editorial 0736 en la base de datos de pubs. New Moon Books se encuentra en Boston, Massachusetts.
Estos son datos de texto de muestra para New Moon Books, editorial 0736 en la base de datos de pubs. New Moon Books se encuentra en Boston, Massachusetts.
Estos son datos de texto de muestra para New Moon Books, editorial 0736 en la base de datos de pubs. New Moon Books se encuentra en Boston, Massachusetts.
...
' Estos son datos de texto de muestra para Algodata Infosystems, editor 1389 en la base de datos de pubs. Algodata Infosystems se encuentra en Berkeley, California.
Estos son datos de texto de muestra para Algodata Infosystems, editor 1389 en la base de datos de pubs. Algodata Infosystems se encuentra en Berkeley, California.
Estos son datos de texto de muestra para Algodata Infosystems, editor 1389 en la base de datos de pubs. Algodata Infosystems se encuentra en Berkeley, California.
Estos son datos de texto de muestra para Algodata Infosystems, editor 1389 en la base de datos de pubs. Algodata Infosystems se encuentra en Berkeley, California.
Estos son datos de texto de muestra para Algodata Infosystems, editor 1389 en la base de datos de pubs. Algodata Infosystems se encuentra en Berkeley, California.
...
insertar valores pub_info('1622',,
Estos son datos de texto de muestra para Five Lakes Publishing, editorial 1622 en la base de datos de pubs. Five Lakes Publishing se encuentra en Chicago, Illinois.
Estos son datos de texto de muestra para Five Lakes Publishing, editorial 1622 en la base de datos de pubs. Five Lakes Publishing se encuentra en Chicago, Illinois.
Estos son datos de texto de muestra para Five Lakes Publishing, editorial 1622 en la base de datos de pubs. Five Lakes Publishing se encuentra en Chicago, Illinois.
Estos son datos de texto de muestra para Five Lakes Publishing, editorial 1622 en la base de datos de pubs. Five Lakes Publishing se encuentra en Chicago, Illinois.
Estos son datos de texto de muestra para Five Lakes Publishing, editorial 1622 en la base de datos de pubs. Five Lakes Publishing se encuentra en Chicago, Illinois.
...
inserte valores roysched ( ' PS1372' , 20001 , 30000 , 14 )
inserte valores roysched ( ' PS1372' , 30001 , 40000 , 16 )
inserte valores roysched ( ' PS1372' , 40001 , 50000 , 18 )

IR

raiserror ( ' Ahora en las inserciones de descuentos ....' , 0 , 1 )

IR

insertar valores de descuento ( ' Cliente inicial' , NULL , NULL , NULL , 10. 5 )
insertar valores de descuento ( ' Descuento por volumen ' , NULL , 100 , 1000 , 6 . 7 )
insertar valores de descuento ( ' Descuento del cliente' , ' 8042 ' , NULL , NULL , 5. 0 )

IR

raiserror ( ' Ahora en las inserciones de trabajos ....' , 0 , 1 )

IR

inserte valores de trabajos ( ' Nueva contratación - Trabajo no especificado' , 10 , 10 )
insertar valores de trabajos ( ' Director Ejecutivo' , 200 , 250 )
insertar valores de trabajos ( ' Business Operations Manager' , 175 , 225 )
inserte valores de trabajos ( ' Chief Financial Officer' , 175 , 250 )
insertar valores de trabajos ( ' Publisher' , 150 , 250 )
insertar valores de trabajos ( ' Editor administrativo' , 140 , 225 )
insertar valores de trabajos ( ' Gerente de Marketing' , 120 , 200 )
insertar valores de trabajos ( ' Gerente de Relaciones Públicas' , 100 , 175 )
insertar valores de trabajos ( ' Gestor de Adquisiciones' , 75 , 175 )
insertar valores de trabajos ( ' Productions Manager' , 75 , 165 )
insertar valores de trabajos ( ' Gerente de operaciones' , 75 , 150 )
insertar valores de trabajos ( ' Editor' , 25 , 100 )
insertar valores de trabajos ( ' Representante de ventas' , 25 , 100 )
insertar valores de trabajo ( ' Diseñador' , 25 , 100 )

IR

raiserror ( ' Ahora en las inserciones al empleado ....' , 0 , 1 )

IR

inserte los valores de los empleados ( ' PTC11962M' , ' Philip' , ' T' , ' Cramer' , 2 , 215 , ' 9952' , '11 /11/89' )
inserte los valores de los empleados ( ' AMD15433F' , ' Ann' , ' M' , ' Devon' , 3 , 200 , ' 9952' , '07 /16/91' )
insertar valores de empleado ( ' F-C16315M' , ' Francisco ' , ' ' , ' Chang' , 4 , 227 , ' 9952 ' , ' 11/03/90' )
inserte los valores de los empleados ( ' LAL21447M' , ' Laurence' , ' A' , ' Lebihan' , 5 , 175 , ' 0736' , '06 /03/90' )
inserte los valores de los empleados ( ' PXH22250M' , ' Paul' , ' X' , ' Henriot' , 5 , 159 , ' 0877' , '08 /19/93' )
inserte los valores de los empleados ( ' SKO22412M' , ' Sven' , ' K' , ' Ottlieb' , 5 , 150 , ' 1389' , '04 / 05/91' )
inserte los valores de los empleados ( ' RBM23061F' , ' Rita' , ' B' , ' Muller' , 5 , 198 , ' 1622' , '10 /09/93' )
insertar valores de empleados ( ' MJP25939M' , ' Maria' , ' J' , ' Pontes' , 5 , 246 , ' 1756' , ' 03/01/89' )
inserte los valores de los empleados ( ' JYL26161F' , ' Janine' , ' Y' , ' Labrune' , 5 , 172 , ' 9901' , '05 /26/91' )
inserte los valores de los empleados ( ' CFH28514M' , ' Carlos' , ' F' , ' Hernadez' , 5 , 211 , ' 9999' , '04 /21/89' )
inserte los valores de los empleados ( ' VPA30890F' , ' Victoria' , ' P' , ' Ashworth' , 6 , 140 , ' 0877' , '09 /13/90' )
inserte los valores de los empleados ( ' L-B31947F' , ' Lesley' , ' ' , ' Brown' , 7 , 120 , ' 0877' , ' 02/13/91' )
inserte valores de empleados ( ' ARD36773F' , ' Anabela ' , ' R ' , ' Domingues ' , 8 , 100 , ' 0877 ' , ' 27/01/93 ' )
inserte los valores de los empleados ( ' M-R38834F' , ' Martine' , ' ' , ' Rance' , 9 , 75 , ' 0877' , ' 05/02/92' )
inserte los valores de los empleados ( ' PHF38899M' , ' Peter' , ' H' , ' Franken' , 10 , 75 , ' 0877' , '05 /17/92' )
...
from titles
donde precio > @lolimit Y precio < @hilimit Y  tipo  = @tipo O  tipo  LIKE  ' %cook%'
agrupar por pub_id, escribir  con  rollup

IR

ACTUALIZAR  ESTADÍSTICAS editores
ACTUALIZAR  ESTADÍSTICAS empleado
ACTUALIZAR  ESTADÍSTICAS trabajos
ACTUALIZAR  ESTADÍSTICAS pub_info
ACTUALIZAR  ESTADÍSTICAS títulos
ACTUALIZAR  ESTADÍSTICAS autores
ACTUALIZAR  ESTADÍSTICAS títuloautor
ACTUALIZAR  ESTADÍSTICAS de ventas
 ESTADÍSTICAS DE ACTUALIZACIÓN roysched
ACTUALIZAR  ESTADÍSTICAS tiendas
ACTUALIZAR  ESTADÍSTICAS descuentos
...
declarar @dttm varchar ( 55 )
seleccione   @dttm = convertir ( varchar , getdate (), 113 )
raiserror ( ' Finalizando InstPubs.SQL en %s ....' , 1 , 1 ,@dttm) sin  esperar
